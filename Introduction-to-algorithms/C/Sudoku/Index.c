...
void buildPlayerGrid(int PlayerGrid[DIMENSION][DIMENSION],
 ...
 * Objetivo: Obtener un numero aleatorio uniforme en el rango [MinRange,
 ...
 *           un generador por backtracking que rellena la matriz con
 * numeros 1..9.
 */
void FillSudokuWithSubMatrix(int Sudoku[DIMENSION][DIMENSION]) {
 ...
 * Argumentos: (int[][]) PlayerMatrix, givens
 *             (int) row, col, cursorRow, cursorCol , selectionMode, candidate
 * Objetivo: Preparar el valor a mostrar y delegar el dibujo al helper de
 * estilo.
 */
void drawCell(int PlayerMatrix[DIMENSION][DIMENSION], int row, int col,
              int cursorRow, int cursorCol, int selectionMode, int candidate,
 ...
 * Argumentos: (int[][]) Sudoku
 * Objetivo: Dibujar en pantalla el estado actual del Sudoku, aplicando colores
 * por bloques 3x3, resaltando la posición del cursor y mostrando valores o
 * candidatos según el modo de selección.
 */
void ShowPlayerMatrix(int PlayerMatrix[DIMENSION][DIMENSION], int cursorRow,
                      int cursorCol, int selectionMode, int candidate,
                      int givens[DIMENSION][DIMENSION]) {
 ...
 * Argumentos: (int[][]) Sudoku
 * Objetivo: Verificar que la matriz 9x9 generada sea valida y no haya numeros
 ...
 */
void colordefault(void) { setcolor(COLOR1, COLOR3); }
