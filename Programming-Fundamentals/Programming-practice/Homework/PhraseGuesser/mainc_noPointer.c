...
int RemoveLetterFromAvailable(char available_letters[], int available_count[1], char letter);
...
void TrackLetter(char letter, char correct_letters[], int correct_count[1],
                 char wrong_letters[], int wrong_count[1], int isCorrect);
void DisplayHangman(int stage);
void PopulateMoneyRoulette(int money_roulette[], int money_roulette_size[1]);
void InitializeAlphabet(char available_letters[], int available_count[1]);
void SelectDisplayedLetters(char available_letters[], int available_count, char displayed_letters[], int display_size);
void AnimateLetterRoulette(char displayed_letters[], int display_count, char selected_letter[1]);
void AnimateMoneyRoulette(int money_roulette[], int money_roulette_size, int selected_value[1]);
void GameInit(int UserInput[1]);
void DisplayGame(int UserInput, char database[][DB_PHRASES][DB_STRING_SIZE],
                 int max_mistakes[1], char selected_phrase[], char original_phrase[]);
 ...
 * Objetivo: Bucle principal de la lógica del juego que maneja la selección de letras,
 *           giros de ruleta y condiciones de victoria/derrota
 * Retorna: 1 si ganó, 0 si perdió, -1 si salió con ESC
 */
int PlayGame(char database[][DB_PHRASES][DB_STRING_SIZE]) {
   int UserInput[1];
   int max_mistakes[1];
   /* selected_phrase ahora es un buffer local; DisplayGame copia la frase aquí */
   char selected_phrase[DB_STRING_SIZE];
   ...
   DisplayGame(UserInput[0], database, max_mistakes, selected_phrase, original_phrase);

   InitializeAlphabet(available_letters, available_count);
   SelectDisplayedLetters(available_letters, available_count[0], displayed_letters, display_size);
   ...
   while (!game_over) {
      ...
      DisplayUI(selected_phrase, money, mistakes, max_mistakes[0],
                correct_letters, correct_count[0], wrong_letters, wrong_count[0],
                displayed_letters, display_size, available_count[0],
                money_roulette, money_roulette_size[0], can_buy);
      ...
      if (key == ESC) {
         ...
         if (!IsLetterInDisplay(spun_letter[0], displayed_letters, display_size)) {
            gotoxy(UI_CONTROLS_X, UI_CONTROLS_Y + 3);
            setcolor(LIGHTRED, BLACK);
         ...
         RemoveLetterFromAvailable(available_letters, available_count, spun_letter[0]);

         int revealed = RevealLetter(selected_phrase, original_phrase, spun_letter[0]);

         if (revealed > 0) {
            TrackLetter(spun_letter[0], correct_letters, correct_count,
                        wrong_letters, wrong_count, 1);

            int money_won[1];
            AnimateMoneyRoulette(money_roulette, money_roulette_size[0], money_won);
            money += money_won[0];

            gotoxy(UI_CONTROLS_X, UI_CONTROLS_Y + 3);
            setcolor(LIGHTGREEN, BLACK);
            printf("Acerto! Letra '%c' revelada %d veces - +$%d%s",
                   spun_letter[0], revealed, money_won[0], CLEAR_SPACE);
            colordefault();
            Sleep(2000);
         } else {
            TrackLetter(spun_letter[0], correct_letters, correct_count,
            ...
            gotoxy(UI_CONTROLS_X, UI_CONTROLS_Y + 3);
            setcolor(LIGHTRED, BLACK);
            printf("Fallo, La letra '%c' no est%c en la frase%s", spun_letter[0], 160, CLEAR_SPACE);
            colordefault();
            Sleep(2000);
         }
         ...
         if (buy_choice == 's' || buy_choice == 'S') {
            ...
            if (strcmp(user_guess, original_phrase) == 0) {
               ...
               money -= buy_cost;
               int lost_opportunities = max_mistakes[0] - mistakes;
 ...
 * Retorna: El carácter en la posición del medio
 */
char GetMiddleLetter(char displayed_letters[], int display_count) {
 ...
 * Retorna: 1 si la letra fue encontrada, 0 en caso contrario
 */
int IsLetterInDisplay(char letter, char displayed_letters[], int display_count) {
 ...
 *             selected_letter - arreglo de tamaño 1 para almacenar la letra final seleccionada
...
void AnimateLetterRoulette(char displayed_letters[], int display_count, char selected_letter[1]) {
   ...
   selected_letter[0] = temp[display_count / 2];

   gotoxy(UI_ROULETTE_X, UI_ROULETTE_Y);
   setcolor(LIGHTGREEN, BLACK);
   printf("Letra seleccionada: %c%s", selected_letter[0], CLEAR_SPACE);
   colordefault();
   Sleep(800);
}
 ...
 *             selected_value - arreglo de tamaño 1 para almacenar el valor de dinero seleccionado
...
void AnimateMoneyRoulette(int money_roulette[], int money_roulette_size, int selected_value[1]) {
   ...
   selected_value[0] = money_roulette[current_index];

   gotoxy(UI_MONEY_ROULETTE_X, UI_MONEY_ROULETTE_Y);
   setcolor(LIGHTGREEN, BLACK);
   printf("Dinero ganado: $%d%s", selected_value[0], CLEAR_SPACE);
   colordefault();
   Sleep(800);
}
 ...
 *             available_count - arreglo de tamaño 1 para almacenar el conteo de letras
 * Objetivo: Inicializa el arreglo de letras disponibles con el alfabeto en minúsculas (a-z)
 */
void InitializeAlphabet(char available_letters[], int available_count[1]) {
 ...
 *             available_count - arreglo de tamaño 1 con el conteo de letras disponibles
 *             letter - la letra a eliminar
 * Objetivo: Elimina una letra específica del conjunto de letras disponibles
 * Retorna: 1 si la letra fue encontrada y eliminada, 0 en caso contrario
 */
int RemoveLetterFromAvailable(char available_letters[], int available_count[1], char letter) {
   for (int ind = 0; ind < available_count[0]; ind++) {
      if (available_letters[ind] == letter) {
         available_letters[ind] = available_letters[available_count[0] - 1];
         available_count[0]--;
         return 1;
      }
   }
 ...
 * Argumentos: hidden_phrase - arreglo con la frase con caracteres ocultos
 *             original_phrase - arreglo con la frase original sin modificar
 *             letter - la letra a revelar en la frase oculta
 * Objetivo: Revela todas las ocurrencias de una letra en la frase oculta
 * Retorna: Número de veces que la letra fue revelada
 */
int RevealLetter(char hidden_phrase[], char original_phrase[], char letter) {
   int count = 0;
 ...
 *             correct_count - arreglo de tamaño 1 con el conteo de letras correctas
 *             wrong_letters - arreglo para almacenar letras incorrectas
 *             wrong_count - arreglo de tamaño 1 con el conteo de letras incorrectas
...
void TrackLetter(char letter, char correct_letters[], int correct_count[1],
                 char wrong_letters[], int wrong_count[1], int isCorrect) {
   /*
    * No se puede usar char[] ni int[] como lvalue en una asignación condicional
    * directamente, así que usamos if/else para seleccionar el arreglo destino.
    */
   if (isCorrect) {
      for (int ind = 0; ind < correct_count[0]; ind++) {
         if (correct_letters[ind] == letter) {
            return;
         }
      }
      correct_letters[correct_count[0]] = letter;
      correct_count[0]++;
   } else {
      for (int ind = 0; ind < wrong_count[0]; ind++) {
         if (wrong_letters[ind] == letter) {
            return;
         }
      }
      wrong_letters[wrong_count[0]] = letter;
      wrong_count[0]++;
   }
}
 ...
 * Argumentos: phrase - arreglo de caracteres de la frase
 * Objetivo: Cuenta el número de letras alfabéticas únicas en una frase
 * Retorna: Conteo de letras únicas (sin distinguir mayúsculas/minúsculas)
 */
int CountUniqueLetters(char phrase[]) {
 ...
 * Retorna: Etapa del ahorcado de 0 a 6
 */
int CalculateHangmanStage(int mistakes, int max_mistakes) {
   if (max_mistakes == 0)
      return 0;
 ...
 * Retorna: El tamaño de ruleta configurado
 */
int GetRouletteConfig(void) {
   int roulette_size;
 ...
 *             money_roulette_size - arreglo de tamaño 1 para almacenar el tamaño de la ruleta
 * Objetivo: Llena la ruleta de dinero con valores aleatorios únicos
 */
void PopulateMoneyRoulette(int money_roulette[], int money_roulette_size[1]) {
   money_roulette_size[0] = (rand() % (MAX_POINTS - MIN_POINT + 1)) + MIN_POINT;

   for (int ind = 0; ind < money_roulette_size[0]; ind++) {
      int already_used;
      int random_value;
...
/**
 * Funcion: GameInit
 * Argumentos: UserInput - arreglo de tamaño 1 para almacenar la selección de tópico del usuario
...
void GameInit(int UserInput[1]) {
   ...
   do {
      scanf("%d", &UserInput[0]);
      if (UserInput[0] <= 0 || UserInput[0] > DB_TOPIC_ID) {
         gotoxy(43, 9);
         setcolor(LIGHTRED, BLACK);
         printf("¡Entre 1 y 5!");
         colordefault();
   ...
   } while (UserInput[0] < 1 || UserInput[0] > DB_TOPIC_ID);
   system("cls");
}

/**
 * Funcion: HidePhrase
 * Argumentos: phrase - arreglo de caracteres de la frase a ocultar
 * Objetivo: Reemplaza todas las letras alfabéticas de la frase por HIDDEN_CHAR
 */
void HidePhrase(char phrase[]) {
   for (int ind = 0; phrase[ind] != '\0'; ind++) {
 ...
 * Retorna: Número máximo de errores permitidos
 */
int CalculateMaxMistakes(char phrase[]) {
 ...
 *             max_mistakes - arreglo de tamaño 1 para almacenar el máximo de errores calculado
 *             selected_phrase - buffer donde se copia la frase seleccionada
...
void DisplayGame(int UserInput, char database[][DB_PHRASES][DB_STRING_SIZE],
   ...
   strcpy(selected_phrase, database[topic_index][random_phrase]);
   strcpy(original_phrase, selected_phrase);
   max_mistakes[0] = CalculateMaxMistakes(selected_phrase);
...
/**
 * Funcion: CheckWin
 * Argumentos: hidden_phrase - arreglo con el estado actual de la frase
 * Objetivo: Verifica si la frase ha sido completamente revelada
 * Retorna: 1 si todas las letras están reveladas, 0 en caso contrario
 */
int CheckWin(char hidden_phrase[]) {
 ...
 * Retorna: Valor en puntos de la letra
 */
int GetLetterValue(char letter) {
 ...
 * Retorna: Valor total de las letras no reveladas
 */
int CalculateRemainingLettersValue(char hidden_phrase[], char original_phrase[]) {
   int total_value = 0;
